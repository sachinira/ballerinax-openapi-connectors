...
public isolated client class Client {
    ...
    # Add attachments to existing feedback.
    #
    # + id - Feedback ID 
    # + payload - Attachment as a binary payload 
    # + return - Success response. 
    remote isolated function postFeedbackAttachments(int id, Body payload) returns FeedbackAttachment|error {
        string  path = string `/feedbacks/${id}/attachments`;
        http:Request request = new;
        FeedbackAttachment response = check self.clientEp->post(path, request, targetType=FeedbackAttachment);
        return response;
    }
